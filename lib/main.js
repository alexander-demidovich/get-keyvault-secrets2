        ...
        try {
            ...
            if (handler != null) {
                ...
                for (var i = 0; i < keyVaultNamesArray.length; i++) {
                    var actionParameters = new KeyVaultActionParameters_1.KeyVaultActionParameters()
                        .getKeyVaultActionParametersForSpecificKeyVaultWithFile(handler, keyVaultNamesArray[i], keyVaultSecretFilesArray[i]);
                    var keyVaultHelper = new KeyVaultHelper_1.KeyVaultHelper(handler, 100, actionParameters);
                    azPath = yield io.which("az", true);
                    var environment = yield executeAzCliCommand("cloud show --query name");
                    environment = environment.replace(/"|\s/g, '');
